...
app.post("/api/request/send", async (req, res) => {
  try {
    ...
    // For Borrow, endDate is also required
...
app.put("/api/request/return/:id", async (req, res) => {
  ...
  try {
    ...
    if (reqDoc.requestType !== "Borrow" || reqDoc.status !== "Accepted") {
      return res.status(400).json({ message: "Not a borrow or not accepted" });
    }

    switch (side) {
      case "owner":
        reqDoc.returnStatusByOwner = true; // owner initiated return request
        break;
      case "sender":
        reqDoc.returnStatusBySender = true; // sender confirms Yes
        break;
      case "ownerConfirm":
        reqDoc.returnedAt = new Date(); // owner confirms returned
        reqDoc.returnStatusByOwner = false; // reset flags
        reqDoc.returnStatusBySender = false;
        break;
      case "ownerDecline":
        reqDoc.returnStatusByOwner = false; // reset to ask again
        reqDoc.returnStatusBySender = false;
        break;
      default:
...
app.put("/api/request/return/:id", async (req, res) => {
  ...
  try {
    ...
    switch (side) {
      case "ownerDirectYes":
        // Owner directly confirms returned
        reqDoc.returnStatusByOwner = true;
        reqDoc.returnStatusBySender = true;
        reqDoc.returnedAt = now;
        reqDoc.declineCount = 0;
        reqDoc.adminuc = false; // Fixed: was adminuc
        break;

      case "owner":
        reqDoc.ownerReturnRequestTime = now;
        break;
      ...
      case "ownerDecline":
        reqDoc.returnStatusByOwner = false;
        reqDoc.returnStatusBySender = false;
        reqDoc.declineCount = (reqDoc.declineCount || 0) + 1;
        if (reqDoc.declineCount >= 3) reqDoc.adminuc = true; // Fixed: was adminuc
        break;
